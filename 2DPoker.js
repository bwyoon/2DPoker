/*
 ...
 남이 만든 거 살짝 바꿔서 자기가 만든 것처럼 하는 사람들이 꼭 있더라고요. 
 ...
 If you find any bug, please report it to Bokwon Yoon at bwyoon@gmail.com 
...
function StartGame()
{
 ...
 if ((iev < 9) && (iev > 0)) {
  alert("Browser support fully HTML5 is recommended." );
 }
 InitGame();
}
...
function ResultDialog()
{
 ...
 if (totalearning > 0) {
  if (totalearning > 25*scalefactor) {
   robj.innerHTML += "Good Job!!!";
  }
  robj.innerHTML += "<br />Your price is "+formatCurrency(totalearning)+".";
  robj.innerHTML += "<div id=\"result-go-doubleup\" onClick=\"ResultDialogGoDoubleUp()\" >Double up</div>"
  robj.innerHTML += "<div id=\"result-no-doubleup\" onClick=\"ResultDialogNoDoubleUp()\" >Take</div>"
 } else {
  robj.innerHTML += "<br /> No Price!!!<br /> ";
  robj.innerHTML += "<div id=\"result-ok\" onClick=\"ResultDialogNoDoubleUp()\" >O.K.</div>"
 }
 ...
 if (mobj.value == "KJK Account") {
  if (localStorage["KJK Account"]) {
   if (localStorage["KJK Account"] == "Yes") {
    alert("You already took momeny from this fund.\nSelect other fund.");
    alreadydid = true;
   }
  } else {
   money = 13000000;
...
function DoubleUpDialogHiLo(opt)
{
 ...
 var str = (num == 7) ? "Even !!.\n" : "Your price is doubled up.\n";

 document.getElementById("doubleup-earning").innerHTML = formatCurrency(totalearning)+"<br />";

 if (totalearning == 0) {
  alert("Your Price is gone.");
  obj.style.zIndex = -3;
  obj.style.display = "none";
  InitGame();
 } else {
  var res=confirm(str+"Double up again?");
  if (!res) {
   AssetChange(totalearning);
   totalearning = 0;
   obj.style.zIndex = -3;
